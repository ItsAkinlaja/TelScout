...
class AIService
{
    ...
    private function resolveProvider(?int $userId): AIProviderInterface
    {
        ...
        $providerName = $settings?->ai_provider ?? config('services.ai.provider', 'openai');
        $apiKey       = $settings?->getAiApiKey();
        $model        = $settings?->ai_model       ?? config('services.ai.model', 'gpt-4o-mini');
        $temperature  = $settings?->ai_temperature ?? (float) config('services.ai.temperature', 0.7);
        $maxTokens    = $settings?->ai_max_tokens  ?? (int)   config('services.ai.max_tokens', 1000);
